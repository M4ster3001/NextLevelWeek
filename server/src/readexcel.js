...
console.clear();
let imp = 'import * as Knex from "knex";'+"\n\n";
let func =    'export async function seed(knex: Knex): Promise<any> {' + "\n"
            + "\t" + 'return knex("cities").insert([' + "\n";

let func_wdel = 'export async function seed(knex: Knex): Promise<any> {' + "\n"
    ...
    rows.forEach( (col) => {
       ...
       if( !oldCity || oldCity != col[3] || j >= 300 ) {
           ...
           if( lines.trim() && oldCity != "UF" ) {
               ...
               fs.writeFileSync( `./database/seeds/cidades_${ oldCity }${ j >= 300 ? '_' + new_city_file : '' }.ts`, 
               ( imp 
                + ( i == 1 ? func_wdel : func ) 
                + lines.trim() 
                + ( i == 1 ? end_wdel : end ) ) );
